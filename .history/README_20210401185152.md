...
See the response from the remote server in below output. You can also see that the requests from the URL are being served by a computer with an IP address .153.133.153 on port 80. More on IP addresses and ports when we get to Networking related projects
